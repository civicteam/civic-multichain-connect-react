...
export interface StandardWalletOptions {
  wallet: Partial<StandardWallet>;
  chains: Chain[];
}

export const standardEthereumWallet = ({
  wallet,
  chains,
  ...options
  ...
  iconBackground: "#fff",
  installed: true,
  chains: chains.map((chain) => `ethereum:${chain.id}`) as IdentifierArray,
  // TODO: Use to dedup.
  hidden: () => false,
  createConnector: () => {
